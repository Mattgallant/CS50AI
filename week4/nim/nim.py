...
class NimAI():
    ...
    def choose_action(self, state, epsilon=True):
        ...
        if epsilon:
            # Epsilon True, use epsilon algo, include random choice chance
            random_value = random.randint(0, 100) / 100
            if random_value <= self.epsilon:
                random_value = random.randint(0, len(actions)-1)
                actions_list = list(actions)
                return actions_list[random_value]
            else:
                for action in actions:
                    if self.get_q_value(state, action) == best_action:
                        return action
        else:
            # Epislon False, no random chance
            for action in actions:
                if self.get_q_value(state, action) == best_action:
                    return action
